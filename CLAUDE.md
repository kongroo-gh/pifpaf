...
踏んだこと:
- **銃と精算パネルを同時に出さない。** 閃光でパネルの文字が読めなくなる。
  銃はラウンド結果の中で見せきり、`MATCH_OVER` では出さない
...
演出まわりで踏んだこと:
- **銃は黒一色だと背景に沈んで見えない。** 盤面がほぼ黒なので、
  明るいガンメタル＋縁の淡い光で浮かせている。暗くし直さないこと
- **銃口の閃光はSVGの中に置く。** CSSで別要素にすると銃口とズレるうえ、
  反動アニメーションについてこない
