...
## トリンカの枚数別ルール（間違えやすい）
4枚組だけ条件が反転している。**3枚組は記号が全て異なること、4枚組は記号の重複が必要。**
7♠7♥7♦7♣ のように4種類すべて違う形は4枚組として認めない（ユーザー指定）。
`melds.test.ts` の「トリンカの枚数別ルール」がこれを固定している。

10枚上がりは 3+3+4 にしか割れないので必ず4枚の役が要る。実測で
4枚の役の内訳は 組30 / 階段10 だったので、重複条件があっても組で作れている。
