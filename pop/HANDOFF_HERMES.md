...
| 表示名 | 画面 `PIF PAF — A TRAVESSIA` ／ ストア名「Pif Paf ほしめぐり」 |

**人間の判断はすべて済んでいる**（`RESKIN_PLAN.md` §7-4 の D1〜D10）。
リスキンの9コミットを止めているものは、もう無い。
...
2. CSS 変数名の改名（値は変えない。**承認済み** → §7-4 D1）
3. 書体の差し替え
4. ポルトガル語の装飾語（§1-1）
...
- **オンラインの入口を消さない。** アプリを単機のみにする決定（D3）は
  Capacitor の段階でビルド時の旗としてやる。リスキンではオンライン側の画面も
  普通にリスキンする
