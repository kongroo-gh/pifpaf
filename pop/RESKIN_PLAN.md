...
- **公開の経路**: `pop/` はどの CI にも繋がっていない。GitHub Pages へ出すなら
  `.github/workflows/` を触るので、AGENTS.md により変更前に承認が要る。
  **この計画には含めない**
- **CSS 変数名を替えるか**（`--felt` → `--deep` など）。替える前提で書いてあるが、
  diff を小さくしたいなら値だけ替えて名前は据え置きでもよい
---
