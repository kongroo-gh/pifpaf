...
### 7-4. 人間の判断（2026-09-07・**D1〜D10 すべて推奨どおりで承認済み**）

| | 決めたこと |
|---|---|
| **D1** CSS 変数名 | **替える**（§2 の対応表）。ただし**値の変更とは別コミット**にして、機械的な改名だけの diff にする |
| **D2** 表示名 | 画面 `PIF PAF — A TRAVESSIA` ／ ストア名 **「Pif Paf ほしめぐり」**。`Pif Paf` は遊びそのものの名前なので残す |
| **D3** 初回リリース | **単機のみ。オンライン対戦は載せない**（→ 実装の注意は下記） |
| **D4** パッケージ名 | **`com.kongroo.pifpaf`**（公開後は変更できない。テーマ語を入れない） |
| **D5** プライバシーポリシー | GitHub Pages に1枚（`https://kongroo-gh.github.io/pifpaf/privacy.html`）。単機のみなので「収集も送信もしない。端末内に設定を5つ保存するだけ」 |
| **D6** 年齢層・IARC | **子ども向け（Families）にはしない**。13歳以上。IARC は実装事実どおり答え、付いたレーティングを受け入れる |
| **D7** レーティング不成立時 | **§7-2 へ進む**（支度・みのりの画面を外す／倍率を廃す）。そのとき条件5の緩和を文書で明示する |
| **D8** 依存の追加 | リスキンが終わってから、**入れるパッケージの一覧を出したうえで**改めて承認を取る |
| **D9** `pop/` の配信 | **当面繋がない**。`pop/` は単体で動くので開発もスクリーンショットも困らない |
| **D10** 現行版 | **当面は併存**。ストアを通ったあとで「`pop/` を本体にするか」を別途決める |

#### D3 の実装上の注意（リスキンの段階ではやらない）

「オンラインを載せない」は**イントロの ONLINE の入口を出さない**ということで、
これは表示の変更にあたる。条件5（画面遷移を変えない）と噛むので、扱いを決めておく。

- **リスキンの9コミットではやらない。** オンライン側の画面も普通にリスキンする
- Capacitor の段階で、**ビルド時の旗**（`import.meta.env` の一つ）で入口の
  出し分けにする。`useOnlineGame` も `OnlineTable` も消さない
- Web 版は今までどおりオンラインが出る。**アプリだけが単機**になる
- この旗を入れるときに、条件5の例外として1行だけ承認を取る
